    ...
    if let Some(&page_id) = pages.get(&1) {
        ...
        if let Ok(font_obj) = res.get(b"Font") {
            ...
            for (name, obj) in fonts.iter() {
                ...
                println!("  Type: {:?}", font_dict.get(b"Type").map(|o| resolve(&doc, o).unwrap()));
                println!("  Subtype: {:?}", font_dict.get(b"Subtype").map(|o| resolve(&doc, o).unwrap()));
                println!("  BaseFont: {:?}", font_dict.get(b"BaseFont").map(|o| resolve(&doc, o).unwrap()));
                println!("  FirstChar: {:?}", font_dict.get(b"FirstChar").map(|o| resolve(&doc, o).unwrap()));
                println!("  LastChar: {:?}", font_dict.get(b"LastChar").map(|o| resolve(&doc, o).unwrap()));
                ...
                if let Ok(desc_obj) = font_dict.get(b"DescendantFonts") {
                    ...
                    println!("  DescendantFont Subtype: {:?}", desc_dict.get(b"Subtype").map(|o| resolve(&doc, o).unwrap()));
