...
pub fn detect_tables(mut pages: Vec<Page>) -> Result<Vec<Page>, ParseError> {
    for page in &mut pages {
        ...
        let mut heights: Vec<f64> = page.blocks.iter().map(|b| {
            let bbox = b.bbox();
            (bbox[3] - bbox[1]).abs()
        }).collect();
        heights.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let font_size = if heights.is_empty() { 12.0 } else { heights[heights.len() / 2] };
        ...
        while i < candidate_rows.len() {
            ...
            while i + table_run < candidate_rows.len() {
                ...
                for b1 in &r1_sorted {
                    for b2 in &r2_sorted {
                        ...
                        let overlap = b1.bbox()[2].min(b2.bbox()[2]) - b1.bbox()[0].max(b2.bbox()[0]);
                        if overlap > 0.0 {
                            matches += 1;
                            break;
                        }
            ...
            for j in 1..=look_range {
                if i >= j {
                    let text = candidate_rows[i - j].blocks.iter().map(|b| b.text().trim()).collect::<Vec<_>>().join(" ").to_lowercase();
            ...
            if !has_caption {
                for j in 0..look_range {
                    if i + table_run + j < candidate_rows.len() {
                        let text = candidate_rows[i + table_run + j].blocks.iter().map(|b| b.text().trim()).collect::<Vec<_>>().join(" ").to_lowercase();
                        if text.contains("table ") || text.starts_with("table") {
                            has_caption = true;
                            break;
                        }
            ...
            if is_table_shape && avg_chars < 30.0 {
                ...
                let source = candidate_rows[i].blocks.first().unwrap().source().to_string();
