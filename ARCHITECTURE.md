...
| lopdf unsupported filter limitation (Known Limitation) | lopdf 0.33 only decodes `FlateDecode` and `LZWDecode` stream filters. Content streams using other filters (`ASCII85Decode`, `ASCIIHexDecode`, `RunLengthDecode`, etc.) silently fall back to raw encoded bytes via `get_page_content()`, producing 0 extractable chars. These pages then get **misrouted to Tier 2 OCR** — the system treats them as scanned pages, producing degraded OCR output instead of the high-quality digital text that should have been extracted. This is a correctness issue, not just a performance one. The most likely filter to encounter in practice is `ASCII85Decode`, used by older PDF generators and reportlab's default compression. | We add our own ASCII85 decoder (a ~40-line algorithm) upstream of `Content::decode()`, or upgrade lopdf to a version that handles additional filters natively. |
---
